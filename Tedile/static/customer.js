...
function providerCard(provider) {
  ...
  const canSchedule = provider.availability !== 'offline';
  ...
  const availabilityLabel = canBook ? 'Available now' : provider.availability === 'busy' ? 'Busy · scheduled bookings available' : 'Offline';
  const action = canSchedule ? `<a class="button button-secondary full-button" href="${escapeHtml(profileHref)}">${canBook ? 'View profile' : 'Schedule a visit'}</a>` : '<span class="button button-secondary full-button" aria-disabled="true">Unavailable for new bookings</span>';
